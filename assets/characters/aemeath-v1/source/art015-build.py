...
(ROOT/'art015-mask-v1.png').write_bytes(maskpath.read_bytes())
...
def tips(rows):
    ...
    return result
protected=[ROOT.parent/'manifest.json',*sorted((ROOT.parent/'frames').glob('*.png')),*sorted((ROOT/'art013-package').rglob('*.*')),*sorted(ROOT.glob('art014-*'))]
...
for version in ["b2-v1","b2-v2","b7-v1","b7-v2"]:
    source=ROOT/f'art015-{version}-source.png'
    ...
    png['write_png'](ROOT/f'art015-{version}-donor.png',96,104,donor)
    assert clipped==0
    out=[bytearray(r) for r in base]
    for x,y in mask:out[y][x*4:x*4+4]=donor[y][x*4:x*4+4]
    dest=ROOT/f'art015-{version}-96.png'
    ...
    report['candidates'][f'{version}']=entry
    ...
    for theme,bg in [('light',(240,239,235,255)),('dark',(29,34,44,255))]:
        for scale in [1,3]:
            ...
            png['write_png'](ROOT/f'art015-{version}-{theme}-{scale}x.png',288*scale,104*scale,output)
assert before=={str(p.relative_to(ROOT.parent)):sha(p) for p in protected}
report['protectedHashesUnchanged']=before
report['references']={name:sha(ROOT/name) for name in ['neutral-generated-v3-edge-cleanup.png','art014-v1-source.png','art012-up-v2-source.png']}
report['pngCrcAndRgbaReadback']='passed for candidate PNGs'
(ROOT/'art015-report.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
