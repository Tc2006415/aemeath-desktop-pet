...
package = json.loads((root.parent / 'manifest.json').read_text(encoding='utf-8'))
images = {frame['path']: 'data:image/png;base64,' + base64.b64encode((root.parent / frame['path']).read_bytes()).decode()
...
html = '''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>ART-006 动画候选检查</title>
...
<h1>ART-006 · 两动作候选检查</h1><p class="warn">待验候选：新帧存在冠尖1px变化和局部重绘差异（闪动风险）；本页不是宿主或游戏逐帧复刻。neutral保持原件。</p>
...
<section><div class="box">浅底3×<br><img class="sprite" id="light"></div><div class="box dark">深底3×<br><img class="sprite" id="dark"></div><div class="box">源帧1×<br><img id="actual"></div></section>
<label>逐毫秒检查 <input id="seek" type="range" min="0" max="1399" value="0"></label><h2>逐帧检查（点击暂停到该项）</h2><div class="strip" id="strip"></div>
<p>idle-soft: neutral → soft-b → soft-a → soft-a保持 → soft-b → neutral；按实际羽饰幅度排序。idle-smile: neutral → half → closed → half → neutral → neutral收势。复用项保留指定时序。</p>
<script>
...
for(let id of ['light','dark','actual'])document.getElementById(id).src=images[f.path];
...
(root / 'idle-inspection.html').write_text(html,encoding='utf-8')
