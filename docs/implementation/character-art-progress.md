...
按PM最新冻结需求，仅引用既有源图/提示/转换与结果，提交source/art024-method-impact.md。纠正先前因果归因：182→10/116是未配准mask合成后的数量，不证明生成源同比缩短；源图三羽仍在，整画布采样未对齐内部位置，固定mask排除了错位翼像素。源形变化/整幅偏移/mask裁切尚不能量化拆分，不指定+5必然可救。

整数整幅donor配准属于用户既有授权，与程序单翼变形/整角色造动作不同；ART024写死零配准是过窄理解。提议PM先裁决现有B2共同固定标志是否支持单一整数偏移及必要时一次新合成，再考虑生成；不直接重置次数。本轮仅方法和范围说明，无生成/合成/新脚本/预览/测试，不重验A、不重跑旧11帧/16入口或格式检查。

## ART-025：现有B2单一整数配准不支持，停止

一次只读标定A与既有B2 donor固定标志：冠尖A(48,22)/donor(48,21)需+1y；嘴中心(47.5,66)/(47.5,64)需+2y；双脚底y93/y90需+3y。冠尖到脚底间距71/69，纯平移不守恒。胸饰色斑重绘，左右固定根部周边亦无法唯一识别同源像素区域，不强行拟合。具体坐标、ROI与可辨性见source/art025-registration-assessment.md。

条件不成立，imagegen0/重合成0，没有尝试偏移或历史+5、没有翼尖拟合、没有新mask/预览/测试脚本。用户选A不重审，不重跑旧11帧/16入口/格式检查；不改PNG/正式包/时序/大幅上扬，不制作C/笑脸。只读坐标标定命令exit0，交最小证据后停止。

## ART026 · 第二版模型待机五核心初版（2026-09-24）

直接执行已确认的初版范围。A为ART023-v2精确字节；3次imagegen，前两源分别因比例和抬幅不采用，第三源双格生成B/C，B/C各1版合成；两笑脸复用相应翼像素。交付source/art026-core/与离线中文art026-demo.html。未进入旧11帧/其他动作、未改正式包或代码。

验证：build.py、preview.py、check.py及5核心Inspect-Png实际执行；96×104 RGBA8 alpha二值，非翼/根部保护、A字节、笑脸翼复用、原1400/1200ms与首尾A、10嵌入原PNG一致。浏览器深浅背景与1/2/3倍、关键帧及播放控制已查；不代表原生验收。B/C各两处对称单像素透明孔保留为初版已知细节，详见art026-delivery.md及check-report；未声称零缺陷。待用户看整体动作后反馈，后续全动作范围等PM派单。

## ART027 · 自然眨眼初版（2026-09-25）

PM记录基础轻扇通过后，仅开展自然眨眼。复用ART009半闭眼，新imagegen1次中性闭眼，两眼姿各1版合成；204像素眼mask，嘴部及所有非眼区RGBA差0，ART026 A/B/C精确保留。交付art027-demo.html、art027-frames/9实际PNG及来源/验证报告。默认单独A翼眨眼，可切换独立时钟叠加轻扇；240ms眨眼、4秒起始间隔仅预览。

实际build/preview/check Python、HTML函数Node时序检查通过：9图格式/CRC/alpha/原字节保护，28000毫秒翼周期比较、70眨眼边界及3逐帧边界。浏览器1/2/3倍、深浅背景、自然闭眼、暂停/逐帧/继续、闭合仍保持C翼已检查。正式包/程序未改，透明点沿用披露，未做原生验收。交回PM等用户审核，停止下一项与自动跟进。

## ART028 · 单眼wink配轻歪头初版

用户确认替代原双眼笑：单眼与同侧轻歪，约1200ms，50–70秒随机等待，拖动禁止且打断回正，完成/释放重计时。本轮imagegen3次（mid1倾斜过大未合成，选mid2+wink1），整体合成1版，候选15PNG及离线art028-demo.html已制作。头饰/头发随头真实绘制倾斜，身体/脚/下翼mask外RGBA差0，原ART026/027文件保持精确字节。原轻扇周期独立，wink与普通眨眼互斥。

实际build/preview/check Python及Node engine检查通过：15图尺寸CRC/RGBA8二值alpha、保护及颈缝、实际嵌入字节；80000毫秒翼轨道、2400wink互斥样本、随机范围/自动触发/模拟拖动屏蔽打断/释放和结束重计时等。浏览器1/2/3倍深浅背景、进入/wink/回正、暂停逐帧与模拟拖动释放重计时已核对；细节及证据限制见art028-delivery.md。头部高光/宝石厚度等生成差异和既有翼透明点披露，尚非生产/原生验收。仅本项，回PM待用户审核，自动跟进暂停。
