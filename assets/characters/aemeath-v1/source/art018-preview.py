"""Self-contained ART-018 timing probe. Never creates or changes sprite pixels."""
import base64
import hashlib
import json
import re
...
protected=[ROOT.parent/'manifest.json',*sorted((ROOT.parent/'frames').glob('*.png')),*sorted(ROOT.glob('art014-*')),*sorted(ROOT.glob('art015-*')),*sorted(ROOT.glob('art016-*')),*sorted(ROOT.glob('art017-*')),*sorted((ROOT/'art013-package').rglob('*.*'))]
before={str(p.relative_to(ROOT.parent)):sha(p) for p in protected}
cycle=[['mid',80],['b2',50],['b4',50],['b7',50],['up',100],['b7',50],['b4',50],['b2',50],['mid',100],['down',140]]
hold=cycle+[x[:] for x in cycle];hold[18]=['closed',100]
...
html='''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>ART-018 动态探针 · 网页模拟</title>
...
<p>暂停项检查（点击任一项）；hold的20项合1440ms，第19项闭眼100ms。</p><div id="strip" class="strip"></div>
<script>
...
function draw(){const t=clock(),s=resolve(t);nowState=s;for(const id of ['light','dark','one','oneDark'])document.getElementById(id).src=images[s.frame];
 ...
 const key=mode.value+':'+s.phase+':'+s.frame+':'+s.index+':'+s.cycles;if(key!==lastKey){trace.push({time:Math.floor(t),mode:mode.value,phase:s.phase,frame:s.frame,index:s.index,cycles:s.cycles,freeze:s.freeze});lastKey=key;window.art018Trace=trace;}
...
window.art018Resolve=resolve;reset();draw();
</script></html>'''
html=html.replace('DATA',json.dumps(data)).replace('IMAGES',json.dumps(images))
dest=ROOT/'art018-inspection.html';dest.write_text(html,encoding='utf-8')
...
(ROOT/'art018-build-report.json').write_text(json.dumps(evidence,indent=2)+'\n',encoding='utf-8')
